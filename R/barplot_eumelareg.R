...
#' @param brewer.palette Color palette from package \code{RColorBrewer} or custom color palette.
#' @param ylab Label for y-axis.
#' @param y.breaks Argument to define breaks on y-axis. Default is \code{breakFun}, which displays the breaks as
#'                 a multiple of 5.
#' @examples dt <- data.table(group = factor(rep(1:6,500)), value = rnorm(3000, 50,15))
